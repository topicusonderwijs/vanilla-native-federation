...
describe('createRemoteEntryProvider', () => {
    ...
    describe('provide', () => {
        it('should fetch and return the remote entry', async () => {
            ...
            const result = await remoteEntryProvider.provide(`${MOCK_REMOTE_ENTRY_SCOPE_I_URL()}remoteEntry.json`);
        ...
        it('should reject with NFError when fetch fails', async () => {
            ...
            await expect(remoteEntryProvider.provide('http://bad.service/remoteEntry.js'))
                .rejects
        ...
        it('should reject with NFError when JSON parsing fails', async () => {                        
            ...
            await expect(remoteEntryProvider.provide(`${MOCK_REMOTE_ENTRY_SCOPE_I_URL()}remoteEntry.json`))
                .rejects
